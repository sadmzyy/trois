...
function usePointer(options) {
  ...
  function getFirstIntersect(intersections) {
    const firstIntersect = intersections.find((intersect2) => {
      ...
      return object2.visible && (object2.userData.selectable !== false || object2 instanceof Mesh$1);
    });
    ...
    if (object) {
      if (object.userData.selectable === false) {
        ...
        object = void 0;
      } else if (object instanceof InstancedMesh$1) {
...
function useThree(params) {
  ...
  function onSelectObject(p) {
    ...
    if (object instanceof InstancedMesh$1 && instanceIdOrFaceIndexId != null && obj.pointer) {
      const im = object;
      ...
      if (clonedMaterial) {
        const clonedMesh = new Mesh$1(im.geometry.clone(), clonedMaterial);
  ...
  function clonedHighlightMaterial(material, options = {}) {
    const { depthTest = void 0, highlightColor = new Color(16711935), opacity = 0.7 } = options;
  ...
  function flyToSelectedObject() {
    ...
    if (sObj instanceof InstancedMesh$1 && sObj.userData.clonedMesh) {
...
class CoordinateAxes extends THREE.Object3D {
  ...
  addTexts() {
    new FontLoader().load("three/fonts/helvetiker_regular.typeface.json", (font) => {
      const x = this.createText(font, "x", new THREE.Color(16711680));
      const y = this.createText(font, "y", new THREE.Color(65280));
      const z = this.createText(font, "z", new THREE.Color(255));
  ...
  createText(font, text, color) {
    ...
    const textMat = new THREE.MeshStandardMaterial({
      ...
      transparent: true,
      opacity: 0.6,
      emissive: color || new THREE.Color(65280)
    });
...
const _ObjectUtils = class {
  ...
  static addOutlines(object, material = this.OUTLINE_MATERIAL.clone(), options = { visibleOnly: true, meshOnly: true, replaceOriginalObject: false }) {
    ...
    if (object.children.length === 0 && (options.visibleOnly && !object.visible || options.meshOnly && !(object instanceof THREE.Mesh))) {
      return [];
    }
    const obj = object;
    ...
    const createOutline = (geometry, matrix) => {
      const geom = new THREE.EdgesGeometry(geometry, 5);
...
ObjectUtils.OUTLINE_MATERIAL = new THREE.LineBasicMaterial({ name: "outline", color: 0, transparent: true, opacity: 0.2 });
...
const _SkyboxUtils = class {
  static createSkyOfGradientRamp(radius = 4e3, widthSegments = 32, heightSegments = 15, skyCenter = new THREE.Vector3()) {
    const uniforms = {
      topColor: { value: new THREE.Color(8828661) },
      skylineColor: { value: new THREE.Color(16777215) },
      bottomColor: { value: new THREE.Color(10066329) },
      offset: { value: 400 },
      exponent: { value: 0.9 },
...
export { AmbientLight, BasicMaterial, BokehPass, Box, BoxGeometry, Geometry as BufferGeometry, PerspectiveCamera as Camera, Circle, CircleGeometry, ComposerInjectionKey, Cone, ConeGeometry, CoordinateAxesViewport, CubeCamera, CubeTexture, Cylinder, CylinderGeometry, DirectionalLight, Dodecahedron, DodecahedronGeometry, EffectComposer, EffectPass, ExportUtils, ExtrudeGeometry, FXAAPass, FBX as FbxModel, FilmPass, GLTF as GltfModel, Group, HalftonePass, HemisphereLight, Icosahedron, IcosahedronGeometry, Image, InstancedMesh, LambertMaterial, Lathe, LatheGeometry, MatcapMaterial, BaseMaterial as Material, MaterialInjectionKey, MaterialUtils, Mesh, MeshInjectionKey, Object3D, ObjectUtils, Octahedron, OctahedronGeometry, OrthographicCamera, PerspectiveCamera, PhongMaterial, PhysicalMaterial, Plane, PlaneGeometry, PmremUtils, PointLight, Points, PointsMaterial, Polyhedron, PolyhedronGeometry, Raycaster, RectAreaLight, RenderPass, Renderer, RendererInjectionKey, Ring, RingGeometry, SMAAPass, SSAOPass, Scene, SceneInjectionKey, SceneUtils, ShaderMaterial, ShadowMaterial, ShapeGeometry, SimplifyUtils, SkyboxUtils, Sphere, SphereGeometry, SpotLight, Sprite, StandardMaterial, SubSurfaceMaterial, Tetrahedron, TetrahedronGeometry, Text, Texture, TiltShiftPass, ToonMaterial, Torus, TorusGeometry, TorusKnot, TorusKnotGeometry, TroisJSVuePlugin, Tube, TubeGeometry, UnrealBloomPass, VideoTexture, Viewer3DUtils, Views, ZoomBlurPass, applyObjectProps, bindObjectProp, bindObjectProps, bindProp, bindProps, createApp, decimalPrecisionRange, getLengthValueByUnit, getMatcapUrl, getUnitStr, lerp, limit, propsValues, setFromProp, showPrecisionValue, unitConversionByMeter, unitLabel, useTextures };
